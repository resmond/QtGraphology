...
from QtGraphology.base.types import TSerializedData
...
from QtGraphology.errors import (
    NodeCreationError,
    NodeDeletionError,
)
from QtGraphology.nodes.backdrop_node import BackdropNode
from QtGraphology.nodes.base_node import BaseNode
from QtGraphology.nodes.group_node import GroupNode
from QtGraphology.nodes.port_node import (
    PortInputNode,
    PortOutputNode,
)
from QtGraphology.widgets.node_graph import (
    NodeGraphWidget,
    SubGraphWidget,
)
...
class NodeGraph(QtCore.QObject):
    ...
    nodes_registered: QtCore.Signal = QtCore.Signal(list)
    """
    Signal triggered when a node is registered into the node graph.

    :parameters: list[:class:`QtGraphology.NodeObject`]
    :emits: registered nodes
    """

    node_created: QtCore.Signal = QtCore.Signal(NodeObject)
    """
    Signal triggered when a node is created in the node graph.

    :parameters: :class:`QtGraphology.NodeObject`
    :emits: created node
    """
    nodes_deleted: QtCore.Signal = QtCore.Signal(list)
    """
    ...
    """
    node_selected: QtCore.Signal  = QtCore.Signal(NodeObject)
    """
    ...
    :parameters: list[:class:`QtGraphology.NodeObject`],
                 list[:class:`QtGraphology.NodeObject`]
    :emits: selected node, deselected nodes.
    """
    node_double_clicked: QtCore.Signal = QtCore.Signal(NodeObject)
    """
    ...
    :emits: input port, output port
    """
    property_changed: QtCore.Signal = QtCore.Signal(NodeObject, str, object)
    """
    Signal is triggered when a property has changed on a node.

    :parameters: :class:`QtGraphology.BaseNode`, str, object
    ...
    """
    context_menu_prompt: QtCore.Signal = QtCore.Signal(object, object)
    """
    Signal is triggered just before a context menu is shown.

    :parameters:
        :class:`QtGraphology.NodeGraphMenu` or :class:`QtGraphology.NodesMenu`,
        :class:`QtGraphology.BaseNode`
    ...
    def _deserialize_context_menu(self, menu, menu_data):
        ...
        Args:
            menu (QtGraphology.NodeGraphMenu or QtGraphology.NodesMenu):
                parent context menu.
    ...
    def _serialize(self, nodes) -> TSerializedData:
        ...
        Args:
            nodes (list[QtGraphology.Nodes]): list of node instances.
    ...
    def _deserialize(self, data: TSerializedData, relative_pos=False, pos=None):
        ...
        Returns:
            list[QtGraphology.Nodes]: list of node instances.
        """
    ...
    def _update_node_rank(node, nodes_rank, down_stream=True):
        ...
        Args:
            node (QtGraphology.BaseNode): node to start from.
    ...
    def _compute_node_rank(nodes, down_stream=True):
        ...
        Args:
            nodes (list[QtGraphology.BaseNode]): nodes to start ranking from.
            down_stream (bool): true to compute down stream.

        Returns:
            dict: {QtGraphology.BaseNode: node_rank, ...}
        """
        nodes_rank = {}
        for node in nodes:
            nodes_rank[node] = 0
...
class SubGraph(NodeGraph):
    ...
    def _clone_context_menu_from_parent(self):
        ...
        def clone_menu(menu, menu_to_clone):
            """
            Args:
                menu (QtGraphology.NodeGraphMenu):
                menu_to_clone (QtGraphology.NodeGraphMenu):
            """
            sub_items = []
